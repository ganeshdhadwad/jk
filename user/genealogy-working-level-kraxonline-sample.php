                                                    ...
                                                    <div class="mgt-item">
                                                        ...
                                                        <div class="mgt-item-parent">
                                                            <div class="person">
                                                                ...
                                                                <p class="name" style="margin-top: -5px"><?php echo $consumeruniqueid;?></p>
                                                            </div>
                                                        </div>
                                                        ...
                                                        <div class="mgt-item-children">
                                                            ...
                                                            <?php
                                                                //check details 
                                                                $checkdtnow = $db->prepare("SELECT * FROM consumer WHERE nomineeid = :nomineeid order by consumerid DESC");
                                                                ...
                                                                while($fetchcount = $checkdtnow->fetch(PDO::FETCH_ASSOC)){
                                                                    ...
                                                                    $fnominee = $fetchcount['nomineeid'];
                                                                    $funiqueid = $fetchcount['uniqueid'];
                                                                     echo $fname.'-'.$fnominee.'-'.$funiqueid.'<br/>';
                                                                     $checkdtnown = $db->prepare("SELECT * FROM consumer WHERE nomineeid = :nomineeid order by consumerid DESC");
                                                                                    ...
                                                                                    while($fetchcountn = $checkdtnown->fetch(PDO::FETCH_ASSOC)){
                                                                                        $fnamen = $fetchcountn['consumerfname'];
                                                                                         $funiqueidn = $fetchcountn['uniqueid'];
                                                                                          $fnomineen = $fetchcountn['nomineeid'];
                                                                                          echo $fnamen.'-'.$fnomineen.'-'.$funiqueidn.'<br/>';
                                                                                     }
                                                                                 }
                                                                    $checkdtnownn = $db->prepare("SELECT * FROM consumer WHERE nomineeid = :nomineeid order by consumerid DESC");
                                                                                                            $checkdtnownn->bindParam(":nomineeid", $funiqueidn);
                                                                                                            $checkdtnownn->execute();
                                                                                                            //count 
                                                                                                            $checkcountnn = $checkdtnownn->rowCount();
                                                                                                            if($checkcountnn > 0){
                                                                                                            while($fetchcountnn = $checkdtnownn->fetch(PDO::FETCH_ASSOC)){
                                                                                                                $fnamenn = $fetchcountnn['consumerfname'];
                                                                                                                $uniquecodenn = $fetchcountnn['uniqueid'];
                                                                                                                $fnomineenn = $fetchcountnn['nomineeid'];
                                                                                                                 echo $fnamenn.'-'.$fnomineenn.'-'.$uniquecodenn.'<br/>';
                                                                                                            }
                                                                                                        }
                                                                }
                                                            ?>
