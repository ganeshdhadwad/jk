...
<head>
    ...
    <!-- Page Title  -->
    <title><?php echo $website_name; ?> Transaction List</title>
    <!-- StyleSheets  -->
...
<?php
    //get user list
    $userlist = $db->prepare("SELECT * FROM wallettoclaim WHERE status = 1 order by walletclaimid DESC");
                                ...
                                <div class="nk-block-head nk-block-head-sm">
                                    <div class="nk-block-between">
                                        <div class="nk-block-head-content">
                                            <h3 class="nk-block-title page-title">Total Transaction Lists</h3>
                                                                ...
                                                                <div>
                                                                    <table class="table table-bordered table-condensed" cellspacing="0" rules="all" border="1" style="width:100%;border-collapse:collapse;">
                                                                        <tbody>
                                                                            ...
                                                                            // $checkdtnow1->execute();
                                                                            $in = '1';
                                                                            $count1 = $userlist->rowCount();
                                                                            while($fetchcount1 = $userlist->fetch(PDO::FETCH_ASSOC)){
                                                                                $userid = $fetchcount1['userid'];
                                                                                $factdaten1 = $fetchcount1['approveddate'];
                                                                                $totalamount = $fetchcount1['totalamount'];
                                                                                $usertstatus3 = $fetchcount1['status'];
                                                                                //get user details 
                                                                                $userdt = $db->prepare("SELECT * FROM consumer WHERE consumerid = :cid");
                                                                                $userdt->bindParam(":cid", $userid);
                                                                                $userdt->execute();
                                                                                //fetch details 
                                                                                $fetchuser = $userdt->fetch(PDO::FETCH_ASSOC);
                                                                                $fname1 = $fetchuser['consumerfname'];
                                                                                $funiqueid1 = $fetchuser['uniqueid'];
                                                                                //$consumeridnnew = $fetchuser['consumerid'];
                                                                                $fsponsorid1 = $fetchuser['nomineeid'];
                                                                                
                                                                                if($fsponsorid1 == ''){
                                                                                    $fsponsorid1n = 'NA';
                                                                                    //echo $fsponsorid1.'--thing';
                                                                                }else{
                                                                                    //echo $fsponsorid1.'--2thing';
                                                                                    $fsponsorid1n = $fsponsorid1;
                                                                                }
                                                                                $crea1 = $fetchuser['created'];
                                                                                //$status1n = $fetchcount1['status'];
                                                                                $created1 = gmdate("d/m/Y", $crea1);
                                                                                $plann1 = $fetchuser['consumerplan'];
                                                                                //$status1n = $fetchcount1['status'];
                                                                                $created1 = gmdate("d/m/Y", $crea1);
                                                                                $plann1 = $fetchuser['consumerplan'];
                                                                                if($plann1 == '1'){
                                                                                ...
                                                                                $status1n = $fetchuser['requestpaid'];
                                                                                if($usertstatus3 == '1'){
                                                                                    $status1m = 'Paid';
                                                                                    $bgname1b = 'badge-success';
                                                                                    $idname1 = 'requestdeactivateuser';
                                                                                }
                                                                                //$factdaten1 = $fetchuser['activationdate'];
                                                                                if($factdaten1 == 0){
                                                                                        $factdate1 = 'NA';
                                                                                        $facttime1 = '';
                                                                                    }else{
                                                                                ...
                                                                                //fetch dt 
                                                                                    if($fsponsorid1 == ''){
                                                                                        $sponsorname1n = 'Nothing';
                                                                                    }else{
                                                                                        $sponsorname1n = $cex->checkSponsorName($fsponsorid1);
                                                                                    }
                                                                                echo '<!-- result starts -->
                                                                                                <tr>
                                                                                                    ...
                                                                                                    <td>
                                                                                                        <span class="tb-sub text-primary" style="text-align: center;">'. $funiqueid1.'</span>
                                                                                                    </td>
                                                                                                    <td style="text-align: center;">
                                                                                                        '. ucfirst($fname1) .'
                                                                                                    </td>
                                                                                                    ...
                                                                                                    <td>
                                                                                                        <div class="'.$bgname1b.'" style="text-align: center; border-radius: 3px; font-size: 11px;">'.$status1m.'</div>
                                                                                                    </td>
                                                                                                    <td>
                                                                                                        '.$totalamount.'
                                                                                                    </td>
